...
for run in run_list:
    ...
    for epoch in tqdm(range(1, epoch_num+1)):
        ...
        val_bacc, val_auc, y_val,y_pred_val= backup(val_loader, model,  epoch, device,2)
        scheduler.step()
    ...
    with torch.no_grad():
        ...
        y_true_2D= F.one_hot(torch.from_numpy(y_true), num_classes=2).cpu().numpy()
    ...
    print('------------------------------------------------------------')
    if 'all' in args.task:
        plot_labels = ['ALL',args.task[0:2]]
    else:
        plot_labels = [args.task[0:3],args.task[5:8]]
