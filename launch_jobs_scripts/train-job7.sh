#!/bin/bash
#SBATCH --job-name=melVSnev # nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=melVSnev_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=melVSnev_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u train_isic_bin.py --task melVSnev 
