#!/bin/bash
#SBATCH --job-name=melVSall # nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=melVSall_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=melVSall_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u train_isic_bin.py --task melVSall 
