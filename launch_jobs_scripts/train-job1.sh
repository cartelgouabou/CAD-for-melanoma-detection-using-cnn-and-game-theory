#!/bin/bash
#SBATCH --job-name=multi # nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=multi_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=multi_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u train_isic_multi.py 
