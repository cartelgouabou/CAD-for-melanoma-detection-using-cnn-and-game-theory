#!/bin/bash
#SBATCH --job-name=prepare # nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=prepare_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=prepare_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u prepare_data_and_models.py 
