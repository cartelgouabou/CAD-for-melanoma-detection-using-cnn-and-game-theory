#!/bin/bash
#SBATCH --job-name=bekVSnev # nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=bekVSnev_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=bekVSnev_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u train_isic_bin.py --task bekVSnev 
