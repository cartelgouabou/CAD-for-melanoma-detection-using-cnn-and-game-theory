...
#Uncertain Melanoma
les_noms = all_scores[(all_scores['M_M'] < 0.5) & (all_scores['M_conf'] > 0.6) ]['image_id'].values
...
for i in range(prev_id, len(image_paths)):
...
les_noms = all_scores[(all_scores['M_M'] > 0.9) & (all_scores['M_conf'] < 0.1) ]['image_id'].values
les_noms = [nom for nom in les_noms if nom in true_mel_names]
print(len(les_noms))

print_path = root_path+"/heat_maps/images/confident/"
