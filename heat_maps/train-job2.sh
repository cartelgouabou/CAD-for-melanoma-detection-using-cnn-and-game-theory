#!/bin/bash
#SBATCH --job-name=unc# nom du job
#SBATCH --partition=gpu_p2 # partition
#SBATCH --qos=qos_gpu-t4 # QoS
#SBATCH --output=unc_%j.out    # fichier de sortie (%j = job ID)
#SBATCH --error=unc_%j.err # fichier d’erreur (%j = job ID)
...
srun python -u uncertain_hm.py --prev_id 60 
